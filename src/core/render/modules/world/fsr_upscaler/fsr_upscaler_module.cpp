...
void FSRUpscalerModule::build() {
    ...
    bool use2xOutput = Renderer::options.outputScale2x && fsr3Enabled_;

    // Output Scale 2x: FSR3 targets 2x display, Lanczos downscales to 1x
    fsr3DisplayWidth_ = use2xOutput ? displayWidth_ * 2 : displayWidth_;
    fsr3DisplayHeight_ = use2xOutput ? displayHeight_ * 2 : displayHeight_;
    ...
    for (uint32_t i = 0; i < size; i++) {
        ...
        if (use2xOutput && initialized_ && upscaled2xImages_[i]) {
...
void FSRUpscalerModule::initLanczosResources() {
    auto fw = framework_.lock();
    uint32_t size = fw->swapchain()->imageCount();
    ...
    lanczosShader_ = vk::Shader::create(
        fw->device(), (Renderer::folderPath / "shaders/world/post_render/lanczos_downscale_comp.spv").string());
...
void FSRUpscalerModuleContext::render() {
    ...
    if (!module->initialized_ || !module->fsr3_) {
        fallbackBlit();
        return;
    }
